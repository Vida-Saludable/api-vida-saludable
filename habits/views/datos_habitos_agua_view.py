...
class DatosHabitosAguaViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        registros_previos = DatosHabitosAgua.objects.filter(usuario_id=usuario_id).order_by('-fecha')
...
class ListaDatosHabitosAguaUsuarioView(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request, usuario_id, *args, **kwargs):
        # Filtrar los registros de hábitos de agua por el usuario recibido en la URL
        registros = DatosHabitosAgua.objects.filter(usuario_id=usuario_id)

        if not registros.exists():
            return Response({
                "success": False,
                "message": "No se encontraron registros de hábitos de agua para este usuario."
            }, status=status.HTTP_404_NOT_FOUND)

        # Serializar los datos
        serializer = DatosHabitosAguaSerializer(registros, many=True)
        return Response({
            "success": True,
            "data": serializer.data
