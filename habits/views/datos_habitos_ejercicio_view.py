...
class DatosHabitosEjercicioViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        registros_previos = DatosHabitosEjercicio.objects.filter(usuario_id=usuario_id).order_by('-fecha')
        ...
        with transaction.atomic():  # Para asegurar que las actualizaciones sean atómicas
            ...
            if serializer.is_valid():
                ...
                response_data = {
                    ...
                    "message": "Registro de ejercicio creado con éxito.",
                ...
                response_data = {
                    "success": False,
                    "message": "Error al crear el registro de ejercicio.",
...
class ListaDatosHabitosEjercicioUsuarioView(APIView):
    ...
    def get(self, request, usuario_id, *args, **kwargs):
        # Filtrar los registros de ejercicio por el usuario recibido en la URL
        registros = DatosHabitosEjercicio.objects.filter(usuario_id=usuario_id)

        if not registros.exists():
            return Response({
                "success": False,
                "message": "No se encontraron registros de hábitos de ejercicio para este usuario."
            }, status=status.HTTP_404_NOT_FOUND)

        # Serializar los datos
        serializer = DatosHabitosEjercicioSerializer(registros, many=True)
        return Response({
            "success": True,
            "data": serializer.data
