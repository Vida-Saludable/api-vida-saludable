...
class ListaDatosHabitosDescansoUsuarioView(APIView):
    def get(self, request, usuario_id, *args, **kwargs):
        # Filtrar los registros por el usuario recibido en la URL
        registros = DatosHabitosDescanso.objects.filter(usuario_id=usuario_id)

        if not registros.exists():
            return Response({
                "success": False,
                "message": "No se encontraron registros de descanso para este usuario."
            }, status=status.HTTP_404_NOT_FOUND)

        # Serializar los datos
        serializer = DatosHabitosDescansoSerializer(registros, many=True)
        return Response({
            "success": True,
            "data": serializer.data
