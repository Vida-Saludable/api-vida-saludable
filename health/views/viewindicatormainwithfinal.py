...
class HealthIndicatorsComparisonAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        usuario_id = self.kwargs.get('usuario_id')
        try:
            datos_iniciales_fisicos = DatosFisicos.objects.filter(usuario_id=usuario_id, tipo='inicial').first()
            datos_finales_fisicos = DatosFisicos.objects.filter(usuario_id=usuario_id, tipo='final').first()

            datos_iniciales_muestras = DatosMuestras.objects.filter(usuario_id=usuario_id, tipo='inicial').first()
            datos_finales_muestras = DatosMuestras.objects.filter(usuario_id=usuario_id, tipo='final').first()

            datos_iniciales_signos = SignosVitales.objects.filter(usuario_id=usuario_id, tipo='inicial').first()
            datos_finales_signos = SignosVitales.objects.filter(usuario_id=usuario_id, tipo='final').first()
        ...
        analisis_comparativo = self.realizar_comparacion(
            datos_iniciales_fisicos, datos_finales_fisicos,
            datos_iniciales_muestras, datos_finales_muestras,
            datos_iniciales_signos, datos_finales_signos,
            datos_personales
        )
    ...
    def realizar_comparacion(self, datos_iniciales_fisicos, datos_finales_fisicos,
        ...
        def calcular_mejora(inicial, final):
            return convertir_a_float(final) - convertir_a_float(inicial)

        def generar_recomendacion(mejora, umbral_mejora, indicador):
            if mejora is None:
                return f"Datos no válidos para {indicador}."

            # Cuando mejora = -inicial (es decir, final == 0 y inicial > 0), asumimos que no hay toma final
            if mejora < 0 and abs(mejora) > 0 and abs(mejora) == abs(mejora + mejora):
                return f"No hay registro del valor final para {indicador}."

            # Cuando mejora == 0, puede ser por falta total de datos (ambos ceros)
            if mejora == 0.0:
                return f"Datos faltantes para {indicador}."
        ...
        def generar_advertencia(inicial, final, indicador, umbral_advertencia):
            ...
            # Si ambos son ceros, sin datos
            if inicial == 0.0 and final == 0.0:
                return f"Datos faltantes para {indicador}."

            # Si solo falta el final
            if inicial > 0 and final == 0.0:
                return f"No hay registro del valor final para {indicador}."
        ...
        def obtener_valor(objeto, campo):
            if objeto is None:
                return 0
            return getattr(objeto, campo, 0)


        indicadores = [
        ('peso', obtener_valor(datos_iniciales_fisicos, 'peso'), obtener_valor(datos_finales_fisicos, 'peso')),
        ('altura', obtener_valor(datos_iniciales_fisicos, 'altura'), obtener_valor(datos_finales_fisicos, 'altura')),
        ('imc', obtener_valor(datos_iniciales_fisicos, 'imc'), obtener_valor(datos_finales_fisicos, 'imc')),
        ('presion_sistolica', obtener_valor(datos_iniciales_signos, 'presion_sistolica'), obtener_valor(datos_finales_signos, 'presion_sistolica')),
        ('presion_diastolica', obtener_valor(datos_iniciales_signos, 'presion_diastolica'), obtener_valor(datos_finales_signos, 'presion_diastolica')),
        ('radio_abdominal', obtener_valor(datos_iniciales_fisicos, 'radio_abdominal'), obtener_valor(datos_finales_fisicos, 'radio_abdominal')),
        ('grasa_corporal', obtener_valor(datos_iniciales_fisicos, 'grasa_corporal'), obtener_valor(datos_finales_fisicos, 'grasa_corporal')),
        ('grasa_visceral', obtener_valor(datos_iniciales_fisicos, 'grasa_visceral'), obtener_valor(datos_finales_fisicos, 'grasa_visceral')),
        ('frecuencia_cardiaca', obtener_valor(datos_iniciales_signos, 'frecuencia_cardiaca'), obtener_valor(datos_finales_signos, 'frecuencia_cardiaca')),
        ('frecuencia_respiratoria', obtener_valor(datos_iniciales_signos, 'frecuencia_respiratoria'), obtener_valor(datos_finales_signos, 'frecuencia_respiratoria')),
        ('colesterol_total', obtener_valor(datos_iniciales_muestras, 'colesterol_total'), obtener_valor(datos_finales_muestras, 'colesterol_total')),
        ('colesterol_hdl', obtener_valor(datos_iniciales_muestras, 'colesterol_hdl'), obtener_valor(datos_finales_muestras, 'colesterol_hdl')),
        ('colesterol_ldl', obtener_valor(datos_iniciales_muestras, 'colesterol_ldl'), obtener_valor(datos_finales_muestras, 'colesterol_ldl')),
        ('trigliceridos', obtener_valor(datos_iniciales_muestras, 'trigliceridos'), obtener_valor(datos_finales_muestras, 'trigliceridos')),
        ('glucosa', obtener_valor(datos_iniciales_muestras, 'glucosa'), obtener_valor(datos_finales_muestras, 'glucosa')),
        ('temperatura', obtener_valor(datos_iniciales_signos, 'temperatura'), obtener_valor(datos_finales_signos, 'temperatura')),
        ('saturacion_oxigeno', obtener_valor(datos_iniciales_signos, 'saturacion_oxigeno'), obtener_valor(datos_finales_signos, 'saturacion_oxigeno')),
        ('porcentaje_musculo', obtener_valor(datos_iniciales_fisicos, 'porcentaje_musculo'), obtener_valor(datos_finales_fisicos, 'porcentaje_musculo')),
        ('glicemia_basal', obtener_valor(datos_iniciales_muestras, 'glicemia_basal'), obtener_valor(datos_finales_muestras, 'glicemia_basal')),
    ]
