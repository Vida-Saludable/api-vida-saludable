...
from django.db.models import Sum, Max, Func, Q
        ...
        try:
            ...
            if fecha_inicial:
                try:
                    ...
                    return Response({'error': 'Formato de fecha_inicial inválido. Use YYYY-MM-DD'}, status=status.HTTP_400_BAD_REQUEST)

            if fecha_final:
                try:
                    date_filters['fecha__lte'] = datetime.strptime(fecha_final, '%Y-%m-%d').date()
                except ValueError:
                    return Response({'error': 'Formato de fecha_final inválido. Use YYYY-MM-DD'}, status=status.HTTP_400_BAD_REQUEST)

            # Obtener queryset base usuarios
            usuarios_filtrados = Usuario.objects.all()

            # Filtrar por nombre si viene
            if nombre:
            ...
            if proyecto_id:
                if proyecto_id == 'todos':
                    if not all_flag:
                        # Solo usuarios que comparten proyectos con usuario autenticado
                        usuario_auth = request.user
                        proyectos_auth = UsuarioProyecto.objects.filter(usuario=usuario_auth).values_list('proyecto_id', flat=True)
                        usuarios_filtrados = usuarios_filtrados.filter(
                            usuarioproyecto__proyecto_id__in=proyectos_auth
                        ).distinct()
                    else:
                        # all_flag=True: no filtrar por proyecto (todos los usuarios)
                        pass
                else:
            ...
            for dormir in registros_dormir:
                ...
                # Filtrar despertar para este usuario y fecha
                despertar_qs = Despertar.objects.filter(
                    usuario_id=usuario_id,
                    fecha__gte=fecha_dormir,
                    hora__gte=dormir['hora']
                ).order_by('fecha', 'hora')

                despertar = despertar_qs.first()
                if not despertar:
                    despertar = Despertar.objects.filter(
                        usuario_id=usuario_id,
