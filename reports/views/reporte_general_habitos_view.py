...
class RegistroHabitosView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            # FILTRO PRINCIPAL: Si no viene all=true, filtrar por proyectos del usuario autenticado
            if not all_flag:
                usuario_auth = request.user
                proyectos_auth_ids = UsuarioProyecto.objects.filter(
                    usuario=usuario_auth
                ).values_list('proyecto_id', flat=True)
                
                usuarios_filtrados = usuarios_filtrados.filter(
                    usuarioproyecto__proyecto_id__in=proyectos_auth_ids
                ).distinct()
