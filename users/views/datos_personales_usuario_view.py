...
class DatosPersonalesUsuarioViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        """
        Sobrescribe retrieve para buscar por usuario_id y devolver la respuesta limpia.
        """
        try:
            datos_personales = DatosPersonalesUsuario.objects.get(usuario_id=pk)
        except DatosPersonalesUsuario.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)
    
        serializer = self.get_serializer(datos_personales)
